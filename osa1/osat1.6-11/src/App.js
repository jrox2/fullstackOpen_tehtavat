...
class App extends React.Component {
  ...
  addListItem = (event) => {
    ...
    if (existingName) {
      ...
      if (window.confirm('on jo luettelossa. Korvataanko vanha numero uudella?')) {
        ...
        console.log('updateThisID: ', updateThisID, 'nameObject.number', nameObject.number)
  ...
  updateListItem = (paramId, nameObject) => {
    nameService
      ...
      .then(response => {
        console.log('responseUpdatessa: ', nameObject)
        this.setState({
      ...
      .catch(error => {
        this.setState({
          error: `henkilö: '${paramId}' on jo valitettavasti poistettu palvelimelta x`,
